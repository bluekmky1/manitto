...
import { getMessagesForUser, getCurrentUser } from "~/utils/supabase.server";

export async function loader({ request }: LoaderFunctionArgs) {
  const userSession = await requireUserSession(request);
  const messages = await getMessagesForUser(userSession.userId);
  const currentUser = await getCurrentUser(userSession.userId);

  return json({ messages, userSession, currentUser });
}
